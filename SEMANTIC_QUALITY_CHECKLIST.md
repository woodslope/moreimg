...
```bash
node semantic-quality-release-check.mjs
```
...
```bash
node semantic-quality-baseline.test.mjs
node semantic-quality-release-check.test.mjs
```
...
3. 按 [semantic-quality-baseline.json](./semantic-quality-baseline.json) 顺序逐篇提交，每篇只点击一次主加工操作。
...
将本轮结果按 [semantic-quality-results.baseline.json](./semantic-quality-results.baseline.json) 的结构记录到一个不含密钥的新 JSON 文件，再运行：

```bash
node semantic-quality-release-check.mjs --verify <本轮结果.json>
```
