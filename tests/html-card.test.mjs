...
assert.match(source, /const HtmlCard = \(\{ card, imageUrl, cardRef, styleLock, focusY \}\) =>/);
assert.match(source, /getCardShellPresentation\(styleLock\)/);
assert.match(source, /const getCardTextDensity = \(card\) =>/);
assert.match(source, /moreimg-card-density-\$\{density\}/);
assert.match(source, /moreimg-card-density-extra-dense/);
assert.doesNotMatch(source, /card\.points\.slice\(/, '高密度卡片不得通过截断要点适配版面');
assert.match(source, /--moreimg-card-focus-y/);
assert.match(source, /aria-label="调整主视觉纵向焦点"/);
assert.match(source, /const saveImageFocus = async/);
assert.match(source, /focusY: item\.focusY/);
assert.match(source, /HTML 排版预览/);
assert.match(source, /等待主视觉/);
...
assert.match(source, /object-position:center var\(--moreimg-card-focus-y,58%\)/);
...
assert.match(source, /不能处理成大片纯色、空雾、无内容墙面或空台面/);
assert.match(source, /避免上下割裂或中间出现无意义空白/);
assert.match(source, /\.moreimg-card-summary\{[^}]*font-size:37px[^}]*font-weight:840/);
assert.match(source, /\.moreimg-card-back \.moreimg-card-summary\{[^}]*font-size:40px/);
assert.match(source, /\.moreimg-card-cover \.moreimg-card-summary,.moreimg-card-back \.moreimg-card-summary\{border-top:0/);
assert.match(source, /\.moreimg-card-cover \.moreimg-card-summary:before,.moreimg-card-back \.moreimg-card-summary:before\{display:block;width:42%/);
assert.match(source, /\.moreimg-card-body \.moreimg-card-summary:before\{display:block;width:28%/);
assert.match(source, /\.moreimg-card-point\{[^}]*border-bottom:0/);
...
const densityBlock = source.match(/const getCardTextDensity[\s\S]*?(?=\n\s*const getDefaultCardFocus)/)?.[0];
assert.ok(densityBlock, '应能提取卡片文字密度计算器');
const getCardTextDensity = Function(`${densityBlock}; return getCardTextDensity;`)();
assert.equal(getCardTextDensity({ type: 'body', title: '先换关系', subtitle: '', points: ['保留判断', '保留论据', '保留条件'], summary: '信息完整' }), 'standard');
assert.equal(getCardTextDensity({ type: 'body', title: '这是一个信息较多但仍需完整保留的标题', subtitle: '', points: ['第一条必要判断不能删除', '第二条关键论据必须保留', '第三条行动条件不能省略', '第四条边界信息仍然重要', '第五条用于形成完整闭环'], summary: '完整表达比机械精简更重要' }), 'extra-dense');
