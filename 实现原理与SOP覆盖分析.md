...
| 层级 | 文件 | 职责 |
| --- | --- | --- |
| JSX 源码 | `src.html` | React 单页应用：表单、请求、阶段解析、结果渲染、历史管理。 |
| 构建脚本 | `build.mjs` | 用本地 Babel 将 `src.html` 内的 JSX 编译到 `app.js`，并生成运行用 `index.html`。 |
| 运行产物 | `index.html`、`app.js` | 浏览器实际加载的页面与编译后逻辑。通常不直接编辑 `app.js`。 |
...
源码修改应优先落在 `src.html`，之后执行 `node build.mjs` 更新 `index.html` 与 `app.js`。
...
- `src.html`：`handleStartProcessing` 负责构造系统消息和用户消息、调用接口、读取普通 JSON 或 SSE 形式响应、保存历史。
- `src.html`：`parseStreamedText` 负责依据“阶段1—阶段6”切分模型文本。
- `src.html`：`renderStageContent` 负责将阶段1/2、3/4、5分别渲染为报告、文章/卡片和提示词复制块。
- `src.html`：`getCleanAllPrompts` 负责从“总览代码块”或拆分版标题中清洗出可复制的整套提示词。
