...
本地页面的外部文本请求会经同源代理转发。`server.py` 会自动探测本机 `7897`、`7890`、`7891` 或 `8080` 端口的 HTTP 代理；如代理端口不同，可先设置 `MOREIMG_UPSTREAM_PROXY=http://127.0.0.1:端口` 再启动服务。

代理只在“请求还没送到上游”的连接故障（连接被拒、主机不可达、DNS 失败）时回退直连，并且按原始参数重建请求——`ProxyHandler` 会就地改写 `Request.host`，沿用同一个对象重试会连回代理端口本身。上游已返回状态码（`HTTPError`）或请求已超时的情况一律不重试：重发会让中转站重复受理、重复计费，还会把真实状态码掩盖成 `502`。超时统一回报 `504`，连接失败回报 `502`，两者都在 `detail` 里带上真实原因，页面据此提示用户先核对用量。

## 测试

全部 21 个 Node 测试（含真实 Chrome 端到端，需要引号让 Node 自己展开 glob）：
...
文本模型每次加工只发起一次 API 请求，请求内容仍是"内置加工规则 + 用户原文 + 简单加工偏好"。核心加工和连接测试都使用 `stream: true`：一次要生成上万 token，非流式期间连接完全静默，中转站前置的 nginx / Cloudflare 会在读超时（常见 60 秒）掐断连接——模型其实已经算完并计费，页面却什么都收不到。持续流出的分片能不断重置网关读超时。响应由统一的缓冲读取器按 `Content-Type` 分流，SSE 和供应商误返回的普通 JSON 都能读，收成整段后只解析一次 `moreimg-1.0` JSON 并渲染，因此不会退回"逐片重渲染整个页面"的性能问题。

模型必须一次返回完整 `moreimg-1.0` JSON；页面严格检查完整正文、封面、连续正文页、固定封底、Style Lock 和逐页视觉字段。允许从单一完整 JSON 外层的代码块或说明文字中安全提取对象，但 JSON 截断、字段缺失、页码跳号或输出结构无法确定时直接显示错误，不自动补写、不自动转换，也不追加文本请求。
...
设置面板新增独立的图片接口地址、图片模型、图片比例和图片 API Key。当前支持兼容 OpenAI Images API 的 `POST /v1/images/generations` 接口；AIXoras/CLIProxy 的 `gpt-image-2` 系列请求包含 `model`、`prompt`、`aspect_ratio` 和 `n: 1`，其他兼容模型使用像素 `size`，响应支持：
...
- 图片比例会按模型协议发送。AIXoras/CLIProxy 的 `gpt-image-2` 系列使用 `aspect_ratio`（支持 `3:4` 等比例，输出像素由上游决定）；其他兼容接口将比例换算为像素 `size`。旧版默认的 `1024x1536` 及早期迁移出的 `2:3` 会在读取配置时统一迁移为 `3:4`，用户重新保存后仍可选择其他支持的比例。
- 接口返回 HTML 错误页时保留状态码与上游原文，不再被 `Unexpected token '<'` 掩盖。
- 设置面板的“生图请求记录（可对账）”按时间记录模型、比例/尺寸、耗时、结果和是否可能已计费，可复制成表格与中转站账单逐条核对。
...
默认生图比例为 `3:4`；AIXoras/CLIProxy 的 `gpt-image-2` 系列通过 `aspect_ratio` 接收该比例，HTML 成品卡固定导出为 `1242x1656`。图片接口的具体输出像素由供应商决定，应用会把生成图作为背景嵌入最终成品卡。
...
该接口返回 `data[0].url` 或 `data[0].b64_json`。模型列表中的名称是 `gpt-image-2`，不是 `image-2`。请求体使用 `aspect_ratio: "3:4"`，不再发送错误的 `size: "1024x1536"`。

本地服务同时提供 `/v1/chat/completions`、`/v1/images/generations`、`/proxy/text`、`/proxy/image`、`/proxy/models` 同源转发，以及 `/proxy/image-asset` 图片本体转发，解决浏览器直接请求中转站和图片 CDN 时的 CORS 限制；转发过程不记录 API Key 或请求正文。带跨站 `Origin` 的转发请求会被 403 拒绝，避免其他网站的页面借这台机器消耗你的 API 额度；无 `Origin` 的请求（curl、测试脚本）照旧放行。

完整的请求、返回、故障证据和调整边界见 [`docs/API调用与返回链路.md`](./docs/API调用与返回链路.md)。
